...
**Goal:** Let an admin add a new Overwatch map (banner + name) via one API call
and have it appear automatically everywhere — website reads, map submission, and
bot slash commands — with **no code change and no redeploy**. Today this needs
four manual steps (make banner → upload to R2 → edit the `OverwatchMap` Literal in
the SDK → insert the DB row); the Literal is the only thing forcing a regen +
redeploy per map. Drop `OverwatchMap = Literal[...]` → `str` + runtime validation
against `maps.names` (+ FK backstop on `core.maps.map_name`); add
`POST /api/v3/content/maps` (mixed-multipart name + banner, `content:admin` scope,
banner stored via a new `maps.names.banner_url` column); add
`GET /utilities/map-names → list[str]` to feed the moderator `MapNameSelect`
dropdown (made DB-fed); replace the plain `INSERT`s in `0001_init.sql` with an
idempotent `ON CONFLICT` seed regenerated by a separate export job (durability),
reconciling the 7 phantom Literal-only maps.

**Requirements:** Grounded in `Skill("spike-findings-genjishimada")` →
`references/dynamic-map-management.md` (spikes 004–008, all VALIDATED). Locked
implementation decisions to be captured in `15-CONTEXT.md` (discuss-phase).
