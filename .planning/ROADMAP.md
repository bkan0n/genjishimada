...
**Goal:** Let an admin add a new Overwatch map (name + required banner) via one
API call and have it appear automatically everywhere — website reads, map
submission, and bot slash commands — with **no code change and no redeploy**. Drop
`OverwatchMap = Literal[...]` → `str` + service-layer runtime validation against
`maps.names` (with a `difflib` "did you mean") + an FK backstop on
`core.maps.map_name`; add `POST /api/v3/content/maps` (mixed-multipart name +
required banner, `content:admin` scope, banner stored at the **stripped-name key**
matching `get_map_banner()` so reads need ZERO change — **no `banner_url` column**,
D-06) doubling as replace-banner, with empty-name + stripped-key collision guards
(422, D-07); add `GET /utilities/map-names → list[str]` feeding the now-DB-fed
moderator `MapNameSelect`; rewrite the `0001_init.sql` seed to one idempotent
`ON CONFLICT` block + a standalone on-demand export script, reconciling the 7
phantom Literal-only maps.

**Requirements:** Locked decisions D-01..D-11 in `15-CONTEXT.md` (source of truth)
+ the REQ-01..REQ-15 map in `15-RESEARCH.md` / `15-VALIDATION.md`. Grounded in
`Skill("spike-findings-genjishimada")` → `references/dynamic-map-management.md`
(spikes 004–008, all VALIDATED). CONTEXT D-04..D-07/D-10 supersede the stale
ROADMAP/Spike `banner_url`-column + backup-wired-export text.
...
- [x] 15-02-PLAN.md — Migration 0032 (phantom reconcile + orphan pre-flight + FK) + idempotent 0001 seed rewrite + standalone export script + schema tests (REQ-11/12/13/14, D-08/09/10/11) [wave 1]

**Wave 2** *(blocked on Wave 1)*

- [x] 15-03-PLAN.md — `upload_map_banner` (stripped key) + MapContentService/Repository: create_map guards (empty + collision → 422) + idempotent insert + validate_map_name; unit tests (REQ-02/05/06/07, D-05/D-07) [wave 2]
