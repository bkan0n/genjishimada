...
| 006 | map-durability | standard | Maps added only to the live DB vanish on rebuild-from-migrations; prove a durable strategy (idempotent re-seed vs. auto-append migration vs. backup-as-truth) | PENDING | maps, migrations, durability |
| 005 | upload-map-live | standard | Running server, no restart: POST name + banner → banner→MinIO, name→`maps.names`, all three surfaces (read / submission / bot autocomplete) update live | PENDING | maps, ui, s3, upload |
