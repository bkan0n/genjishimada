...
class TestFetchLeaderboard:
    ...
    async def test_leaderboard_verified_tier_outranks_pending_video(
        ...
        map_id = await create_test_map()
        cycle_id = await create_test_cycle(category_id, map_id, status="active")
        user_a = await create_test_user(nickname="VerifiedNoVideo")
        user_b = await create_test_user(nickname="PendingVideo")

        # Invariant preserved: `verified` (mod-approval) stays the top axis above
        # `completion`. A faster, video-bearing run that is still PENDING must rank
        # below a mod-verified no-video run -- the edition drain logic depends on
        # pending runs sorting below verified ones.
        await create_test_tournament_completion(cycle_id, user_a, map_id, time=30.0, verified=True)
        await create_test_tournament_completion(
            cycle_id, user_b, map_id, time=20.0, status="pending", video="https://example.com/run.mp4"
        )
