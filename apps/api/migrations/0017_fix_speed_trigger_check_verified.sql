-- Fix speed enforcement trigger to check against ALL non-legacy completions,
-- not just unverified ones. Previously, verified completions were invisible
-- to the trigger, allowing slower submissions after a faster verified run.
...
BEGIN
    ...
    -- Find the best non-legacy run for this user/map (verified AND unverified).
