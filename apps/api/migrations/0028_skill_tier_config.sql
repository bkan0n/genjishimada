...
--              boundaries (icon ranks 1..7, 0 = Unranked) layered on top of the
--              existing numeric skill_score (migration 0027). The `percentiles`
--              array is the ONLY tunable — boundaries are DERIVED from the live
--              distribution by recompute_all (no hardcoded score cutoffs, mirroring
--              the 0027 "no hardcoded weights" rule). Boundaries start EMPTY and are
--              filled by the first qualifying recompute; reads treat an empty
...
-- Single-row tier config (one row, like skill.weight_config): the 6 computed cut-point
-- scores, the 6 configurable percentiles that produce them, and when they were computed.
