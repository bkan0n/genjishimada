...
class NotificationService(BaseService):
    ...
    async def create_and_dispatch(
        ...
        event = await self._create_event(data)
    ...
    async def _create_event(self, data: NotificationCreateRequest) -> NotificationEventResponse:
        """Store notification event in database."""
        query = """
            INSERT INTO notifications.events (user_id, event_type, title, body, metadata)
            VALUES ($1, $2, $3, $4, $5)
            RETURNING id, user_id, event_type, title, body, metadata,
                      created_at, read_at, dismissed_at
        """
        metadata_json = msgspec.json.encode(data.metadata).decode() if data.metadata else None
    ...
    async def _get_enabled_channels(self, user_id: int, event_type: NOTIFICATION_EVENT_TYPE) -> list[str]:
        ...
        query = """
            SELECT channel, enabled FROM notifications.preferences
            WHERE user_id = $1 AND event_type = $2
        """
        rows = await self._conn.fetch(query, user_id, event_type)

        # Build preference map
        explicit_prefs = {row["channel"]: row["enabled"] for row in rows}
    ...
    async def record_delivery_result(
        ...
        query = """
            ...
            ON CONFLICT (event_id, channel) DO UPDATE SET
                ...
                error_message = $4
        """
        await self._conn.execute(query, event_id, channel, status, error_message)
    ...
    async def get_user_events(
        ...
        *,
        ...
        rows = await self._conn.fetch(base_query, user_id, limit, offset)
    ...
    async def mark_read(self, event_id: int) -> None:
        ...
        await self._conn.execute(query, event_id)
    ...
    async def mark_all_read(self, user_id: int) -> int:
        ...
        query = """
            ...
            RETURNING id
        """
        result = await self._conn.fetch(query, user_id)
        return len(result)
    ...
    async def update_preference(
        ...
        """
        await self._conn.execute(query, user_id, event_type, channel, enabled)
    ...
    def _row_to_event_response(self, row) -> NotificationEventResponse:  # noqa: ANN001
...
async def provide_notification_service(conn: Connection, state: State) -> NotificationService:
    """Litestar DI provider for NotificationService."""
    return NotificationService(conn, state)
