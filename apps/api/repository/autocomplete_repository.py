from __future__ import annotations

import logging
from typing import cast

from asyncpg import Connection
from genjishimada_sdk.maps import Mechanics, OverwatchCode, OverwatchMap, PlaytestStatus, Restrictions
from litestar.datastructures import State

from .base import BaseRepository

log = logging.getLogger(__name__)


class AutocompleteRepository(BaseRepository):
    async def get_similar_map_names(
        self, search: str, limit: int = 5, *, conn: Connection | None = None
    ) -> list[OverwatchMap] | None:
        """Get similar map names.

        Args:
            search (str): The input string to compare.
            limit (int, optional): Maximum number of results. Defaults to 5.
            conn: Database connection.

        Returns:
            list[OverwatchMap] | None: A list of matching map names or `None` if no matches are found.

        """
        _conn = self._get_connection(conn)

        query = "SELECT name FROM maps.names ORDER BY similarity(name, $1::text) DESC LIMIT $2;"
        res = await _conn.fetch(query, search, limit)
        if not res:
            return None
        return [r["name"] for r in res]

    async def transform_map_names(self, search: str, *, conn: Connection | None = None) -> OverwatchMap | None:
        """Transform a map name into an OverwatchMap.

        Args:
            search (str): Input string to transform.
            conn: Database connection.

        Returns:
            OverwatchMap | None: The closest matching map name, or `None` if no matches are found.

        """
        _conn = self._get_connection(conn)

        query = "SELECT name FROM maps.names ORDER BY similarity(name, $1::text) DESC LIMIT 1;"
        res = cast("OverwatchMap", await _conn.fetchval(query, search))
        if res is None:
            return None
        return f'"{res}"'  # type: ignore

    async def get_similar_map_restrictions(
        self, search: str, limit: int = 5, *, conn: Connection | None = None
    ) -> list[Restrictions] | None:
        """Get similar map restrictions.

        Args:
            search (str): Input string to compare.
            limit (int, optional): Maximum number of results. Defaults to 5.
            conn: Database connection.

        Returns:
            list[Restrictions] | None: Matching restriction names, or `None` if no matches are found.

        """
        _conn = self._get_connection(conn)
        query = "SELECT name FROM maps.restrictions ORDER BY similarity(name, $1::text) DESC LIMIT $2;"
        res = await _conn.fetch(query, search, limit)
        if not res:
            return None
        return [r["name"] for r in res]

    async def transform_map_restrictions(self, search: str, *, conn: Connection | None = None) -> OverwatchMap | None:
        """Transform a map name into a Restriction.

        Args:
            search (str): Input string to transform.
            conn: Database connection.

        Returns:
            Restrictions | None: The closest matching restriction, or `None` if no matches are found.

        """
        _conn = self._get_connection(conn)

        query = "SELECT name FROM maps.restrictions ORDER BY similarity(name, $1::text) DESC LIMIT 1;"
        res = cast("Restrictions", await _conn.fetchval(query, search))
        if res is None:
            return None
        return f'"{res}"'  # type: ignore

    async def get_similar_map_mechanics(
        self, search: str, limit: int = 5, *, conn: Connection | None = None
    ) -> list[Mechanics] | None:
        """Get similar map mechanics.

        Args:
            search (str): Input string to compare.
            limit (int, optional): Maximum number of results. Defaults to 5.
            conn: Database connection.

        Returns:
            list[Mechanics] | None: Matching mechanics, or `None` if no matches are found.

        """
        _conn = self._get_connection(conn)

        query = "SELECT name FROM maps.mechanics ORDER BY similarity(name, $1::text) DESC LIMIT $2;"
        res = await _conn.fetch(query, search, limit)
        if not res:
            return None
        return [r["name"] for r in res]

    async def transform_map_mechanics(self, search: str, *, conn: Connection | None = None) -> Mechanics | None:
        """Transform a map name into a Mechanic.

        Args:
            search (str): Input string to transform.
            conn: Database connection.

        Returns:
            Mechanics | None: The closest matching mechanic, or `None` if no matches are found.

        """
        _conn = self._get_connection(conn)

        query = "SELECT name FROM maps.mechanics ORDER BY similarity(name, $1::text) DESC LIMIT 1;"
        res = cast("Mechanics", await _conn.fetchval(query, search))
        if res is None:
            return None
        return f'"{res}"'  # type: ignore

    async def get_similar_map_codes(  # noqa: PLR0913
        self,
        search: str,
        archived: bool | None = None,
        hidden: bool | None = None,
        playtesting: PlaytestStatus | None = None,
        limit: int = 5,
        *,
        conn: Connection | None = None,
    ) -> list[OverwatchCode] | None:
        """Get similar map codes.

        Args:
            search (str): Input string to compare.
            archived (bool | None, optional): Filter by archived flag, or `None` for no filter.
            hidden (bool | None, optional): Filter by hidden flag, or `None` for no filter.
            playtesting (PlaytestStatus | None, optional): Filter by playtesting status, or `None` for no filter.
            limit (int, optional): Maximum number of results. Defaults to 5.
            conn: Database connection.

        Returns:
            list[OverwatchCode] | None: Matching map codes, or `None` if no matches are found.

        """
        _conn = self._get_connection(conn)

        query = """
            SELECT code FROM core.maps
            WHERE ($2::bool IS NULL OR archived = $2) AND
            ($3::bool IS NULL OR hidden = $3) AND
            ($4::playtest_status IS NULL OR playtesting = $4)
            ORDER BY
                CASE
                    WHEN code = $1::text THEN 3
                    WHEN code ILIKE $1::text || '%' THEN 2
                    ELSE similarity(code, $1::text)
                END DESC
            LIMIT $5;
        """

        res = await _conn.fetch(query, search, archived, hidden, playtesting, limit)
        if not res:
            return None
        return [r["code"] for r in res]

    async def transform_map_codes(
        self,
        search: str,
        archived: bool | None = None,
        hidden: bool | None = None,
        playtesting: PlaytestStatus | None = None,
        *,
        conn: Connection | None = None,
    ) -> OverwatchCode | None:
        """Transform a map name into a code.

        Args:
            search (str): Input string to transform.
            archived (bool | None, optional): Filter by archived flag, or `None` for no filter.
            hidden (bool | None, optional): Filter by hidden flag, or `None` for no filter.
            playtesting (PlaytestStatus | None, optional): Filter by playtesting status, or `None` for no filter.
            conn: Database connection.

        Returns:
            OverwatchCode | None: The closest matching map code, or `None` if no matches are found.

        """
        _conn = self._get_connection(conn)

        query = """
            SELECT code FROM core.maps
            WHERE ($2::bool IS NULL OR archived = $2) AND
            ($3::bool IS NULL OR hidden = $3) AND
            ($4::playtest_status IS NULL OR playtesting = $4)
            ORDER BY
                CASE
                    WHEN code = $1::text THEN 3
                    WHEN code ILIKE $1::text || '%' THEN 2
                    ELSE similarity(code, $1::text)
                END DESC
            LIMIT 1;
        """
        res = cast("OverwatchCode", await _conn.fetchval(query, search, archived, hidden, playtesting))
        if res is None:
            return None
        return f'"{res}"'  # type: ignore

    async def get_similar_users(
        self,
        search: str,
        limit: int = 10,
        fake_users_only: bool = False,
        ignore_fake_users: bool = False,
        *,
        conn: Connection | None = None,
    ) -> list[tuple[int, str]] | None:
        """Get similar users by nickname, global name, or Overwatch username.

        Args:
            search (str): Input string to compare.
            limit (int, optional): Maximum number of results. Defaults to 10.
            fake_users_only (bool): Filter out actually discord users and display fake members only.
            ignore_fake_users: Ignore fake users
            conn: Database connection.

        Returns:
            list[tuple[int, str]] | None: A list of `(user_id, display_name)` tuples, or `None` if no matches are found.

        """
        _conn = self._get_connection(conn)

        query = """
        WITH matches AS (
            SELECT u.id AS user_id, name, similarity(name, $1) AS sim
            FROM core.users u
            CROSS JOIN LATERAL (
                VALUES (u.nickname), (u.global_name)
            ) AS name_list(name)
            WHERE ($3 IS FALSE OR id < 1000000000000000) AND ($4 IS FALSE OR id > 1000000000000000)

            UNION ALL

            SELECT o.user_id, o.username AS name, similarity(o.username, $1) AS sim
            FROM users.overwatch_usernames o
            WHERE ($3 IS FALSE OR user_id < 1000000000000000) AND ($4 IS FALSE OR user_id > 1000000000000000)
        ),
        ranked_users AS (
            SELECT user_id, MAX(sim) AS sim
            FROM matches
            GROUP BY user_id
            ORDER BY sim DESC
            LIMIT $2
        ),
        user_names AS (
            SELECT
                u.id AS user_id,
                ARRAY_REMOVE(
                    ARRAY[u.nickname, u.global_name] || ARRAY_AGG(owu.username),
                    NULL
                ) AS all_usernames
            FROM ranked_users ru
            JOIN core.users u ON u.id = ru.user_id
            LEFT JOIN users.overwatch_usernames owu ON owu.user_id = u.id
            GROUP BY u.id, u.nickname, u.global_name, ru.sim
            ORDER BY ru.sim DESC
        )
        SELECT
            user_id,
            CASE
                WHEN array_length(all_usernames, 1) = 1 THEN all_usernames[1]
                ELSE
                    all_usernames[1] || ' (' ||
                    array_to_string(
                        ARRAY(SELECT DISTINCT unnest(all_usernames[2:array_length(all_usernames, 1)])), ', ') || ')'
            END AS name
        FROM user_names;
        """
        res = await _conn.fetch(query, search, limit, fake_users_only, ignore_fake_users)
        if not res:
            return None
        return [(r["user_id"], r["name"]) for r in res]


async def provide_autocomplete_repository(state: State) -> AutocompleteRepository:
    """Litestar DI provider for AutocompleteRepository.

    Args:
        state (State): Application state.

    Returns:
        AutocompleteRepository: A new service instance bound to the given connection.

    """
    return AutocompleteRepository(state.db_pool)
